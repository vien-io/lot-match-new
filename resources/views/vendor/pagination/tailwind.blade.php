...
<nav role="navigation" aria-label="{{ __('Pagination Navigation') }}" class="tw-flex tw-items-center tw-justify-between">
    ...
    {{-- Desktop --}}
    <div class="tw-gap-10 tw-hidden sm:tw-flex-1 sm:tw-flex sm:tw-items-center sm:tw-justify-between">
        <div>
        ...
        <div>
            <span class="tw-relative tw-z-0 tw-inline-flex tw-rtl:tw:flex-row-reverse tw-shadow-sm tw-rounded-md">
                {{-- Previous Page Link --}}
                @if ($paginator->onFirstPage())
                ...
                @else
                    <a href="{{ $paginator->previousPageUrl() }}" rel="prev" class="tw-relative tw-inline-flex tw-items-center tw-px-2 tw-py-2 tw-text-sm tw-font-medium tw-text-gray-500 tw-bg-white tw-border tw-border-gray-300 tw-rounded-l-md tw-leading-5 tw-hover:tw-text-gray-400 focus:tw-z-10 focus:tw-outline-none focus:tw-ring tw-ring-gray-300 focus:tw-border-blue-300 tw-active:tw-bg-gray-100 tw-active:tw-text-gray-500 dark:tw-bg-gray-800 dark:tw-border-gray-600 dark:tw-active:tw-bg-gray-700 dark:focus:tw-border-blue-800" aria-label="{{ __('pagination.previous') }}">
                        <svg class="tw-w-5 tw-h-5" fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd" d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z" clip-rule="evenodd"/>
                        </svg>
                    </a>
                ...
                @foreach ($elements as $element)
                    ...
                    @if (is_array($element))
                        @foreach ($element as $page => $url)
                            ...
                            @else
                                <a href="{{ $url }}" class="tw-relative tw-inline-flex tw-items-center tw-px-4 tw-py-2 -tw-ml-px tw-text-sm tw-font-medium tw-text-gray-700 tw-bg-white tw-border tw-border-gray-300 tw-leading-5 tw-hover:tw-text-gray-500 focus:tw-z-10 focus:tw-outline-none focus:tw-ring tw-ring-gray-300 focus:tw-border-blue-300 tw-active:tw-bg-gray-100 tw-active:tw-text-gray-700 dark:tw-bg-gray-800 dark:tw-border-gray-600 dark:tw-text-gray-400 dark:tw-hover:tw-text-gray-300 dark:tw-active:tw-bg-gray-700" aria-label="{{ __('Go to page :page', ['page' => $page]) }}">
                                    {{ $page }}
                                </a>
                            @endif
                        @endforeach
